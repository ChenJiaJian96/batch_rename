...
class MyGUI:
    ...
    def set_init_window(self):
        ...
        self.open_xls_button.place(relx=0.8, rely=0.5, relwidth=0.15, relheight=0.15)
        self.confirm_button.place(relx=0.8, rely=0.8, relwidth=0.15, relheight=0.15)
    ...
    def update_table_by_dict(self):
        for i in range(len(self.table_name_list0)):
            if self.name_reflect_dict.keys().__contains__(self.table_name_list0[i]):
                self.table_name_list1.append(self.name_reflect_dict[self.table_name_list0[i]])
            else:
    ...
    def set_cell_value(self, event):  # 双击进入编辑状态
        ...
        def save_edit():
            try:
                self.tree_view.set(item, column=column, value=entry_edit.get(0.0, "end"))
            except NameError:
...
class ExcelMaster:
    ...
    def get_name_dict(self):
        ...
        print(self.table.col_types(i, start_rowx=1, end_rowx=None))
        print(self.table.col_types(j, start_rowx=1, end_rowx=None))
